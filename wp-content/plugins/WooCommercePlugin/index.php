<?php
/*
Plugin Name: ALRAJHI Payment Gateway
Plugin URI: https://alrajhibank.com.sa
Description: Extends WooCommerce with Al Rajhi Bank.
Version: 4.3.0
Author: FSSNeT
Author URI: https://www.fssnet.co.in/
Copyright: © 2020 FSSNeT. All rights reserved.
*/
if ( ! defined( 'ABSPATH' ) )
{
    exit;
...
add_action('plugins_loaded', 'woocommerce_alrajhi_init', 0);

function woocommerce_alrajhi_init() {

  if ( !class_exists( 'WC_Payment_Gateway' ) ) return;  
  /**
   * Localisation
   */
  load_plugin_textdomain('wc-alrajhi', false, dirname( plugin_basename( __FILE__ ) ) . '/languages');
  
  if($_GET['msg']!=''){
    add_action('the_content', 'showalrajhiMessage');
  }

  function showalrajhiMessage($content){
    return '<div class="box '.htmlentities($_GET['type']).'-box">'.htmlentities(urldecode($_GET['msg'])).'</div>'.$content;
  }
  /**
   * Gateway class
   */
  class WC_Alrajhi extends WC_Payment_Gateway {
    protected $msg = array();
    ...
    public function __construct(){
      ...
      // Go wild in here
      $this -> id = 'alrajhi';
      $this -> method_title = __('Al Rajhi', 'alrajhi');
	  $this -> method_description = __('Pay securely by Credit or Debit card or net banking through ALRAJHI.', 'alrajhi');
      $this -> icon = WP_PLUGIN_URL . "/" . plugin_basename(dirname(__FILE__)) . '/images/alrajhilogo.jpg';
      $this -> has_fields = false;
      $this -> init_form_fields();
      $this -> init_settings();
      $this -> title = 'Al Rajhi'; //$this -> settings['title'];
      ...
      add_action('init', array(&$this, 'check_alrajhi_response'));
      //update for woocommerce >2.0
      add_action( 'woocommerce_api_' . strtolower( get_class( $this ) ), array( $this, 'check_alrajhi_response' ) );

      add_action('valid-alrajhibob-request', array(&$this, 'SUCCESS'));
      ...
      add_action('woocommerce_receipt_alrajhi', array(&$this, 'receipt_page'));
      add_action('woocommerce_thankyou_alrajhi',array(&$this, 'thankyou_page'));
    ...
    function init_form_fields(){
      ...
      $this -> form_fields = array(
        'enabled' => array(
            'title' => __('Enable/Disable', 'alrajhi'),
            'type' => 'checkbox',
						'label' => __('Enable Al Rajhi', 'alrajhi'),
            'default' => 'no'),
		  'encryption' => array(
            'title' => __('Encryption Method', 'alrajhi'),
            'type' => 'select',
            'options' => array("aes"=>"AES-IV","tdes"=>"TDES"),
            'description' => __('AES-IV or Tripple DES','alrajhi')
            ),
		  'description' => array(
			'title' => __('Description:', 'alrajhi'),
			'type' => 'textarea',
			'description' => __('This controls the description which the user sees during checkout.', 'alrajhi'),
			'default' => __('Pay securely by Credit or Debit card or net banking through Al Rajhi.', 'alrajhi')),
          'gateway_url' => array(
            'title' => __('Gateway URL', 'alrajhi'),
            'type' => 'textarea',
            'description' => __('The URL to the hosted payment page of ALRAJHI.', 'alrajhi'),
			'default' => __('https://securepayments.alrajhibank.com.sa/pg/PaymentHTTP.htm?param=paymentInit', 'alrajhi')
            ),
		  'tranportalid' => array(
            'title' => __('Portal ID', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('Portal ID as provided', 'alrajhi')
            ),
		  'tranportalpassword' => array(
            'title' => __('Portal Password', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('Portal Password as provided.', 'alrajhi')
            ),
		  'termresourcekey' => array(
            'title' => __('Terminal Resource Key', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('Resource Key as provided.', 'alrajhi')
            ),
		  'udf1' => array(
            'title' => __('UDF1', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),
		  'udf2' => array(
            'title' => __('UDF2', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),
		  'udf3' => array(
            'title' => __('UDF3', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),
		  'udf4' => array(
            'title' => __('UDF4', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),
		  'udf5' => array(
            'title' => __('UDF5', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),
		  'udf6' => array(
            'title' => __('UDF6', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),
		  'udf7' => array(
            'title' => __('UDF7', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),
		  'udf8' => array(
            'title' => __('UDF8', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),
		  'udf9' => array(
            'title' => __('UDF9', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),	
		  'udf10' => array(
            'title' => __('UDF10', 'alrajhi'),
            'type' => 'text',
            'description' =>  __('User Defined Field for custom parameters.', 'alrajhi')
            ),	
    ...
    public function admin_options(){
      echo '<h3>'.__('ALRAJHI', 'alrajhi').'</h3>';
      echo '<p>'.__('ALRAJHI most popular payment gateways for online shopping.').'</p>';
    ...
    function receipt_page($order){
      echo '<p>'.__('Thank you for your order, please wait as you will be automatically redirected to ALRAJHI.', 'alrajhi').'</p>';
      echo $this -> generate_alrajhi_form($order);
    }
     ...
     **/    
    function check_alrajhi_response(){
			...
			try
			{
				...
				if (isset($ResResult) && strtoupper($ResResult) == 'CAPTURED' && $ResAuthRespCode == '00') {
					...
					else
					{
						...
						$order -> add_order_note('Al Rajhi has processed the payment. Ref Number: '.$ResPaymentId);
						$order -> add_order_note($this->msg['message']);
						$order -> add_order_note("Paid by Al Rajhi");
     ...
     **/    
    public function generate_alrajhi_form($order_id){
   ...
   **/
  function woocommerce_add_alrajhi_gateway($methods) {
    $methods[] = 'WC_Alrajhi';
    return $methods;
  }

  add_filter('woocommerce_payment_gateways', 'woocommerce_add_alrajhi_gateway' );
