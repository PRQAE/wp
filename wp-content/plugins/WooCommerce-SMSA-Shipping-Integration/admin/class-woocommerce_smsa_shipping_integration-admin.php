...
class woocommerce_smsa_shipping_integration_Admin {
	...
	public function add_meta_boxes(){
		...
		if(in_array($post_type, $order_types)){
			...
			add_meta_box( 'smsa-order-manager', __( 'بيانات سمسا', 'ced-umb-smsa' ) . wc_help_tip( __( 'Send product data to SMSA server for tracking number.', 'ced-umb-smsa' ) ), array($this,'order_manager_box') );
		}
		...
		if(!is_null($order_id)){
			...
			if(is_wp_error($order)){
				...
				if(!empty($tracking_number)){
					echo"<span>رقم التتبع : $tracking_number</span>";
					echo"<button type='button' data-orders=".$order_id." data_order_cancle=".$tracking_number." class='button-primary ced_smsa_cancle_shipment' >إلغاء الشحنة</button>";
					$smsa_file_name = get_post_meta($order_id , 'smsa_shipping_pfd' , true);
					if (!empty($smsa_file_name)) {
						echo"<a target='_blank' type='button' href='admin-ajax.php?action=ced_smsa_get_pfd_download&filename=$smsa_file_name&oid=$orderId' class='button-primary ced_order_get_pdf' data_order_id=".$order_id." >تحميل البوليصة</a>";
					}else{
						echo"<button type='button' data_order_get_pdf=".$tracking_number." class='button-primary ced_smsa_get_invoice' data_order_id=".$order_id." >طلب البوليصة</button>";
					}
				}else{
					echo"<button type='button' data-order=".$order_id." class='button-primary ced_smsa_send_data' >إرسال إلى سمسا</button>";
				}
		...
		if(!empty($woo_order_id)){
			...
			try {
				...
				if(!empty($awbno) && is_numeric($awbno)){
					...
					$order->update_status('smsa');
					if(isset($id) && !empty($id)){
						return $id;
					}else{

						echo"تم إرسال البيانات إلى سمسا بنجاح";
					}
				}else{
					if(isset($id) && !empty($id)){
						return $awbno;
	...
	public function mwb_smsa_new_order_column($columns){
		$columns['ced_sma_invoice'] = __('بوليصة سمسا','ced_smsa');
		return $columns;
	}
	...
	public function mwb_smsa_wc_add_data_invoice_column_content($column){
		...
		switch ( $column ) 
		{
			...
			if(!empty($tracking_number)){
				...
				if (!empty($smsa_file_name)) {
					echo"<a target='_blank' href='admin-ajax.php?action=ced_smsa_get_pfd_download&filename=$smsa_file_name&oid=$orderId' class='button-primary' data_order_id=".$orderId." >تحميل البوليصة</a>";
				}else{
					echo"<a href='admin-ajax.php?action=ced_smsa_get_pfd&tracking_number=$tracking_number&oid=$orderId' class='button-primary' data_order_id=".$orderId." >طلب بوليصة</a>";
				}
			}else{
