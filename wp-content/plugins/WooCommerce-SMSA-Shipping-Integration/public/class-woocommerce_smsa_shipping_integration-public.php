...
class woocommerce_smsa_shipping_integration_Public {
	...
	public function my_tracking_smsa($columns){
		if(is_array($columns) && !empty($columns)){
			...
			array("smsa_tracking_num" => __('رقم التتبع' , 'ced_smsa')) +
	...
	public function smsa_tracking_num_order_detail_page($order){ 
		...
		<h2 class="ced_tracking_title"> بيانات تتبع الشحنة </h2>
		<table class="woocommerce-table ced_tracking_table">
			<tr>
				<td> رقم التتبع </td>
				<td> <?php if(isset($ced_smsa_tracking_no) && $ced_smsa_tracking_no != ""){ echo '<a href="http://www.smsaexpress.com/Track.aspx?tracknumbers='.$ced_smsa_tracking_no.'" target="_blank">'.$ced_smsa_tracking_no.'</a>';  } else{ echo "جاري تجهيز الشحنة";} ?> </td>
				</tr>
			</table>
		</div>
		<?php
